...
CHAT_RESPONSES = {
    "love": [
        ...
        "Венера подчёркивает важность маленьких жестов. Не жди грандиозных поводов — тёплое слово сегодня стоит больше, чем букет через неделю.",
    ...
    "generic": [
        "Звёзды редко дают однозначные ответы, но всегда дают направление. Твоя интуиция сейчас особенно точна — прислушайся к тому, что уже знаешь внутри.",
        "По натальному чарту вижу: ты уже знаешь ответ. Звёзды лишь подтверждают то, что подсказывает сердце.",
        "Хороший вопрос. Планеты сейчас говорят: действуй из места любви, а не страха — и результат тебя порадует.",
    ],
}
...
def get_chat_response(text: str) -> str:
    t = text.lower()
    if re.search(r"муж|партн|люб|роман|чувств|отнош", t):
        return random.choice(CHAT_RESPONSES["love"])
    if re.search(r"дет|ребён|ребенок|семь|мам|свекр|дочь|сын", t):
        return random.choice(CHAT_RESPONSES["family"])
    if re.search(r"здоров|болез|самочувств|устал|сил|энерг", t):
        return random.choice(CHAT_RESPONSES["health"])
    if re.search(r"деньг|финанс|куп|расход|бюджет|работ|доход", t):
        return random.choice(CHAT_RESPONSES["finance"])
    if re.search(r"когда|время|ждать|скоро|период", t):
